...
Before me make the visualizations lets work out how to crop the plot to a circle. The main idea here is to save the plot to a temporary file, fix its size with some extra space around it. Then we draw a circle around the plot and crop it to that circle. The function allows us to control the width and height of the cropped plot which will come in handy when cropping different plots.
...
crop_to_circle <- function(original_plot, width, height) {
  ...
  img <- image_read(temp_file) |>
      ...
      image_border("none", "10x10") |>
      image_trim()
    
    # Create circular mask
    mask <- image_draw(image_blank(500, 500))
    symbols(240, 220, circles = 220, bg = 'black', inches = FALSE, add = TRUE)
    dev.off()
...
crop_to_circle(sleep_plot, 12.5, 12.5)
```
...
crop_to_circle(heart_plot, 15, 15)
```
...
geom_link2(data = grid_line2, 
    ...
    #add the box
    annotate(geom = "text", x =median(stress_data$time), y = 120, label = 25, color = "grey90", size = 80
) +
    annotate(geom = "text", x =median(stress_data$time), y = 110, label = paste0("rest"), color = "grey60", size = 50
) +
geom_richtext(aes(x = median(stress_data$time), y = 125, label = "<span style='font-family:fa-solid'>&#xf007;</span>"), text.color = "blue", size = 90, fill = NA) +
...
coord_cartesian(ylim = c(0, 135), xlim = c(1, 68)) +
...
crop_to_circle(stress_plot, 15, 15)
```
...
crop_to_circle(training_plot, 15, 15)
```
